...
from xpdtools.utils import ProposalIDPrompt
    ...
    RE = RunEngine(
        {
            ...
            "cycle": f"{datetime.today().year}-{datetime.today().month % 4 + 1}",
        }
    )
...
if XPDTOOLS_RUNNING_IN_CI:
    path_provider._beamline_proposals_dir = Path(mkdtemp())  # noqa: SLF001
